    ...
    public class BaitManager : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            for (int i = 0; i < fish.Count; i++)
            {
                ...
                _baitDataArray[i] = new BaitData(bait.transform.position, bait.GetScriptable().areaOfEffect, bait.GetScriptable().GetFoodTypes(), fish[i].transform.position, (int)fish[i].GetComponent<Edible>().FoodType);
